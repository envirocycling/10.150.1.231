...
if (isset($_GET['action']) && $_GET['action'] == 'submitForm') {
    ...
    if ($prepaid == '1') {
        if (mysql_query("INSERT INTO `adv`(`ac_no`, `supplier_id`, `branch_id`, `amount`, `acpty_id`, `acty_id`, `justification`, `terms`,`prepaid`,`status`,`user_id`,`verified_id`,`verified_date`,`approved_id`,`approved_date`,`date`)
        VALUES ('$ac_no','$supplier_id','7','$amount','$acpty_id','$acty_id','$justification','$terms','$prepaid','verified','$user_id','$user_id','$date','$user_id','$date','$date')")) {
        ...
        if (mysql_query("INSERT INTO `adv`(`ac_no`, `supplier_id`, `branch_id`, `amount`, `acpty_id`, `acty_id`, `justification`, `terms`,`prepaid`,`status`,`user_id`,`verified_id`,`verified_date`,`date`)
        VALUES ('$ac_no','$supplier_id','7','$amount','$acpty_id','$acty_id','$justification','$terms','$prepaid','verified','$user_id','$user_id','$date','$date')")) {
            if (mysql_query("UPDATE adv_sysgen_no SET sup_nx_ctrl_no='$new_no'")) {
                //do something
            } else {
                $err++;
...
//        $old_cheque_no = "";
//    }
//    if (mysql_query("UPDATE temp_payment SET bank_code='" . $_POST['account'] . "', cheque_no='" . $_POST['cheque_no'] . "', old_cheque_no='$old_cheque_no', voucher_no='" . $_POST['voucher_no'] . "', cheque_name='$cheque_name', sub_total='', ts_fee='', adjustments='', grand_total='" . $_POST['grand_total'] . "', type='" . $_POST['type'] . "', ap='" . $_SESSION['user_id'] . "', verifier='" . $_SESSION['verifier'] . "', signatory='" . $_SESSION['signatory'] . "',cheque_date='" . $_POST['cheque_date'] . "' WHERE user_id='" . $_SESSION['user_id'] . "'")) {
//
//        $particular = mysql_real_escape_string(strtoupper(utf8_encode($_POST['particular'])));
//        if (mysql_query("UPDATE `temp_payment_others` SET `particulars` =  '$particular',`quantity` =  '" . $_POST['quantity'] . "',`unit_price` =  '" . $_POST['unit_price'] . "',`amount` =  '" . $_POST['amount'] . "' WHERE  `user_id`='" . $_SESSION['user_id'] . "' and `others_count` =  '1'")) {
//            //do something
//        } else {
//            $err++;
//        }
...
//        }
//
//        if (mysql_query("INSERT INTO payment (`bank_code`, `cheque_no`, `voucher_no`, `cheque_name`, `supplier_id`, `sub_total`, `ts_fee`, `adjustments`, `grand_total`, `type`, `pay_type`, `ap`, `verifier`, `signatory`, `date`,`cheque_date`)
//    VALUES ('" . $rs_save['bank_code'] . "','" . $rs_save['cheque_no'] . "','" . $rs_save['voucher_no'] . "','$cheque_name','" . $_POST['supplier_id'] . "','" . $rs_save['sub_total'] . "','" . $rs_save['ts_fee'] . "','" . $rs_save['adjustments'] . "','" . $rs_save['grand_total'] . "', '" . $rs_save['type'] . "', 'Advances', '" . $rs_save['ap'] . "', '" . $rs_save['verifier'] . "', '" . $rs_save['signatory'] . "','$my_date', '" . $rs_save['cheque_date'] . "')")) {
...
//                    if (mysql_query("INSERT INTO payment_others (payment_id,particulars,quantity,unit_price,amount) VALUES ('$payment_id','$particular','" . $rs_save_adj['quantity'] . "','" . $rs_save_adj['unit_price'] . "','" . $rs_save_adj['amount'] . "')")) {
//                        //do something
//                    } else {
//                        $err++;
//                    }
...
//    if ($rs_count == 0) {
//
//        if (mysql_query("INSERT INTO payment (`bank_code`, `cheque_no`, `voucher_no`, `cheque_name`, `supplier_id`, `sub_total`, `ts_fee`, `adjustments`, `grand_total`, `type`, `pay_type`, `cancelled_cheque`, `date`, `cheque_date`)
//    VALUES ('" . $rs_save['bank_code'] . "','" . $rs_save['cheque_no'] . "','" . $rs_save['voucher_no'] . "','$cheque_name','" . $_POST['supplier_id'] . "','" . $rs_save['sub_total'] . "','" . $rs_save['ts_fee'] . "','" . $rs_save['adjustments'] . "','" . $rs_save['grand_total'] . "', '" . $rs_save['type'] . "', 'Advances','" . $rs_save['old_cheque_no'] . "', '" . date("Y/m/d") . "','" . $rs_save['cheque_date'] . "')")) {
//
//            $sql_save_adj = mysql_query("SELECT * FROM temp_payment_others WHERE user_id='" . $_SESSION['user_id'] . "'");
//            while ($rs_save_adj = mysql_fetch_array($sql_save_adj)) {
//                if (!empty($rs_save_adj['particulars'])) {
...
//                        if (mysql_query("INSERT INTO payment_others (payment_id,particulars,quantity,unit_price,amount) VALUES ('" . $_POST['payment_id'] . "','$particular','" . $rs_save_adj['quantity'] . "','" . $rs_save_adj['unit_price'] . "','" . $rs_save_adj['amount'] . "')")) {
//                            //do something
//                        } else {
//                            $err++;
//                        }
//                    } else {
//                        if (mysql_query("UPDATE payment_others SET particulars='$particular',quantity='" . $rs_save_adj['quantity'] . "',unit_price='" . $rs_save_adj['unit_price'] . "',amount='" . $rs_save_adj['amount'] . "' WHERE id='" . $rs_save_adj['others_id'] . "'")) {
//                            //do something
//                        } else {
//                            $err++;
//                        }
...
//        }
//
//        if (mysql_query("INSERT INTO payment (`bank_code`, `cheque_no`, `voucher_no`, `cheque_name`, `supplier_id`, `sub_total`, `ts_fee`, `adjustments`, `grand_total`, `account_name`, `account_number`, `type`, `pay_type`, `ap`, `verifier`, `signatory`, `date`,`cheque_date`)
//    VALUES ('" . $rs_save['bank_code'] . "','" . $rs_save['cheque_no'] . "','" . $rs_save['voucher_no'] . "','$cheque_name','" . $_POST['supplier_id'] . "','" . $rs_save['sub_total'] . "','" . $rs_save['ts_fee'] . "','" . $rs_save['adjustments'] . "','" . $rs_save['grand_total'] . "','" . $rs_save['account_name'] . "','" . $rs_save['account_number'] . "', '" . $rs_save['type'] . "', 'Advances', '" . $rs_save['ap'] . "', '" . $rs_save['verifier'] . "', '" . $rs_save['signatory'] . "','$my_date', '" . $rs_save['cheque_date'] . "')")) {
//            $sql_save_adj = mysql_query("SELECT * FROM temp_payment_others WHERE user_id='" . $_SESSION['user_id'] . "'");
//            while ($rs_save_adj = mysql_fetch_array($sql_save_adj)) {
//                if (!empty($rs_save_adj['particulars'])) {
//                    $particular = mysql_real_escape_string(strtoupper($rs_save_adj['particulars']));
...
//        }
//        // im herre
//        mysql_query("UPDATE adv SET payment_id='$payment_id',status='issued' WHERE ac_id='" . $_GET['ac_id'] . "'");
//    }
//}
