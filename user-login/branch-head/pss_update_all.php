...
$sql_rec = mysql_query("SELECT *,scale.status as status1,supplier.status as status2 FROM scale INNER JOIN supplier ON scale.supplier_id=supplier.supplier_id WHERE (scale.company_id='1' or scale.company_id='2' or scale.company_id='5') and scale.status='COMPLETED' and scale.date>='$today' and scale.date<='$today' and supplier.owner='EFI'");
...
foreach ($data as $item) {
    ...
    if (!$pss_del_res) {
        ...
        if ($last_row_res) {
            ...
            //increment
            $day_code = sprintf("%05d", ($_day_code + 1));

            $code = $year_code . $month_code . '-' . $day_code;

            $nxt_code = $year_code . $month_code . '-' . ($day_code + 1);
            ...
            $nxt_code = $year_code . $month_code . '-' . $day_code . '002';

        }

        $queryStr = "update `pss_delivery_code` set `code`='$nxt_code' where id=1;";
...
header('Location: pss_delivery_performance_reports.php');
