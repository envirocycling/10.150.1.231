...
<?php
include 'config.php';
if (isset($_GET['sic_id'])) {
    mysql_query("UPDATE users SET status='deleted' WHERE user_id='".$_GET['sic_id']."'");
}
...
$sql_user = mysql_query("SELECT * FROM users WHERE usertype='5' and status!='deleted'");
while ($rs_user = mysql_fetch_array($sql_user)) {
    ...
    echo "<td class='data'>";
    ?>
<a href="query_sic.php?sic_id=<?php echo $rs_user['user_id']; ?>" onclick="return confirm('Are you sure you want to delete?')"><button>Delete</button></a>
    <?php
    echo "</td>";
    echo "</tr>";
}
