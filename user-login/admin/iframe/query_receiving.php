...
//        window.open("../request_adjustment.php?trans_id=" + str, 'mywindow', 'width=700,height=300,left=150,top=50');
        window.open("../edit_transaction.php?trans_id=" + str, 'mywindow', 'width=700,height=500,left=150,top=50');
...
if (!isset($_GET['from'])) {
    $sql_rec = mysql_query("SELECT * FROM scale_receiving WHERE date>='" . date("Y/m/d") . "' and date<='" . date("Y/m/d") . "' and status!='void'");
} else {
...
while ($rs_rec = mysql_fetch_array($sql_rec)) {
    ...
    echo "<a href=\"../delete_receiving.php?trans_id=" . $rs_rec['trans_id'] . "\"><button class='submit' onclick=\"return confirm('Do you want to Delete?');\">Delete</button></td>";
    echo "</tr>";
}
echo "</table>";
?>