...
include 'config.php';
if (isset($_GET['ap_id'])) {
    mysql_query("UPDATE users SET status='deleted' WHERE user_id='".$_GET['ap_id']."'");
}
...
$sql_user = mysql_query("SELECT * FROM users WHERE usertype='4' and status!='deleted'");
while ($rs_user = mysql_fetch_array($sql_user)) {
    ...
    echo "<td class='data'>";
    ?>
<a href="query_ap.php?ap_id=<?php echo $rs_user['user_id']; ?>" onclick="return confirm('Are you sure you want to delete?')"><button>Delete</button></a>
    <?php
    echo "</td>";
    echo "</tr>";
}
