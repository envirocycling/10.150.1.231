...
<html>
                ...
                ?>" size="10" readonly required></td>
                                <td>&nbsp;&nbsp;&nbsp;Branch:
                                    <select name="branch" class="tcal">
										...
										$sql_branch = mysql_query("SELECT * from branches WHERE status != 'offline'") or die(mysql_error());
                    ...
                    if (isset($_POST['submit'])) {
						...
						$sql_branches = mysql_query("SELECT * from branches WHERE branch_name LIKE '%$branch%' and status != 'offline'") or die(mysql_error());
                        ...
                        while ($row_branches = mysql_fetch_array($sql_branches)) {
                            ...
                            if (200 == $retcode) {
                                ...
                                if ($row_branches['branch_id'] == 7) {
                                  ...
                                  $sqli = mysqli_connect("$url_sqli", 'branches', 'enviro101', "$db_name");
                                ...
                                while ($row_suppAdv = mysqli_fetch_array($sql_suppAdv)) {
                                    ...
                                    while ($row_amountPaid = mysqli_fetch_array($sql_amountPaid)) {
                                        ...
                                        if (mysqli_num_rows($sql_payment) > 0) {
											...
											if (!isset($amount_paid[$row_branches['branch_id']][$row_suppAdv['supplier_id']][$row_suppAdv['date_processed']])){
												$amount_paid[$row_branches['branch_id']][$row_suppAdv['supplier_id']][$row_suppAdv['date_processed']] = 0;
											}
                                            $amount_paid[$row_branches['branch_id']][$row_suppAdv['supplier_id']][$row_suppAdv['date_processed']] += $row_amountPaid['amount'];
                                        }
                                    }

                                    $sql_lastdeductionadj = mysqli_query($sqli, "SELECT * from payment_adjustment WHERE ac_id='" . $row_suppAdv['ac_id'] . "' and adj_type='deduct' and amount > 0 ORDER BY adj_id Desc");
                                    ...
                                    while ($row_lastdeductionadj = mysqli_fetch_array($sql_lastdeductionadj)) {
                                        ...
                                        if (mysqli_num_rows($sql_lastdeduction) == 1) {
                                            ...
                                            $date_paid1 = date('Y/m/d', strtotime($row_lastdeduction['date']));

                                            $sql_cash = mysqli_query($sqli, "SELECT * from adv_payment WHERE paid_date < '$date_paid1' and can_id='0' and paid_date != '0000-00-00 00:00:00' and amount > 0");
                                            if (mysqli_num_rows($sql_cash) == 1) {
                                                $row_cash = mysqli_fetch_array($sql_cash);
                                                $last_deduction[$row_branches['branch_id']][$row_suppAdv['supplier_id']][$row_suppAdv['date_processed']] = 'Date:' . date('Y/m/d', strtotime($row_cash['paid_date'])) . ' Amount:' . number_format($row_cash['amount'], 2);
                                                break;
                                            } else {
                                                $last_deduction[$row_branches['branch_id']][$row_suppAdv['supplier_id']][$row_suppAdv['date_processed']] = 'Date:' . date('Y/m/d', strtotime($row_lastdeduction['date'])) . ' Amount:' . number_format($row_lastdeductionadj['amount'], 2);
                                                break;
                                            }
                                        }
                                    }
    ...
    foreach ($branch_arr as $slctd_branch) {
        ...
        if ($row_branches['branch_id'] != 7) {
            ...
            $db_name = 'efi_pamp';
            $sqli2 = mysqli_connect("$url", 'branches', 'enviro101', "$db_name");
        ...
        while ($row_suppAdv2 = mysqli_fetch_array($sql_suppAdv2)) {
			...
			if (!isset($amount_paid[$slctd_branch][$row_suppAdv2['supplier_id']][$row_suppAdv2['date_processed']])){
				$amount_paid[$slctd_branch][$row_suppAdv2['supplier_id']][$row_suppAdv2['date_processed']] = 0;
			}
			...
			if (!isset($remarks_arr[$slctd_branch][$row_suppAdv2['supplier_id']][$row_suppAdv2['date_processed']])){
				$remarks_arr[$slctd_branch][$row_suppAdv2['supplier_id']][$row_suppAdv2['date_processed']] = "";
			}
            $remarks = $remarks_arr[$slctd_branch][$row_suppAdv2['supplier_id']][$row_suppAdv2['date_processed']];
			if (!isset($last_deduction[$slctd_branch][$row_suppAdv2['supplier_id']][$row_suppAdv2['date_processed']])){
				$last_deduction[$slctd_branch][$row_suppAdv2['supplier_id']][$row_suppAdv2['date_processed']] = 0;
			}
