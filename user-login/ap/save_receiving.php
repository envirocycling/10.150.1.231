...
if (isset($_POST['submit'])) {
    ...
    $save_by = $_SESSION['ap_id'];

    mysql_query("UPDATE scale_receiving SET save_by='$save_by' WHERE trans_id='" . $_POST['trans_id'] . "'");
    ...
    if (isset($_POST['forEval'])) {
        mysql_query("UPDATE scale_receiving SET str_no='" . $_POST['str_no'] . "',tr_no='" . $_POST['tr_no'] . "',series_no='" . $_POST['series_no'] . "',supplier_id='" . $_POST['supplier_id'] . "',status='for_evaluation',upload='0',up_paper='0',checked='0' WHERE trans_id='" . $_POST['trans_id'] . "'");
    } else {
        mysql_query("UPDATE scale_receiving SET str_no = '" . $_POST['str_no'] . "',tr_no='" . $_POST['tr_no'] . "', series_no = '" . $_POST['series_no'] . "', supplier_id = '" . $_POST['supplier_id'] . "', status = 'generated', upload = '0', up_paper = '0', checked = '1' WHERE trans_id = '" . $_POST['trans_id'] . "'");
    }

    mysql_query("UPDATE scale_outgoing SET str_no = '" . $_POST['str_no'] . "',tr_no = '" . $_POST['tr_no'] . "', series_no = '" . $_POST['series_no'] . "', supplier_id = '" . $_POST['supplier_id'] . "', upload = '0', up_out = '0', checked = '1' WHERE rec_trans_id = '" . $_POST['trans_id'] . "'");
    $c = $_POST['ctr'];
    $ctr = 0;
    while ($ctr < $c) {
        if($_POST['del_to'] == '1' || $_POST['del_to'] == '2'){
            $delTo = '1';
        }else{
            $delTo = $_POST['del_to'];
        }
//        echo "SELECT * FROM suppliers_price WHERE dt_id='".$_POST['del_to']."' and material_id='" . $_POST['mat_' . $ctr] . "' and supplier_id='" . $_POST['supplier_id'] . "' and date<='" . $rs_trans['date'] . "' ORDER BY date DESC";
        $sql_sup_price = mysql_query("SELECT * FROM suppliers_price WHERE dt_id='$delTo' and material_id='" . $_POST['mat_' . $ctr] . "' and supplier_id='" . $_POST['supplier_id'] . "' and date<='" . $rs_trans['date'] . "' ORDER BY date DESC");
    ...
    echo "<center>";
    echo "<h2>Save Receiving</h2>";
    echo "<form action='save_receiving.php' method='POST'>";
    ...
    echo "</tr>";
    echo "<tr>";
    echo "<td>TR Number: </td>";
    echo "<td><input type='text' name='tr_no' value='" . $rs_trans['tr_no'] . "'></td>";
    echo "</tr>";
    echo "<tr>";
    ...
    while ($rs_details = mysql_fetch_array($sql_details)) {
        ...
        echo "<td>";
//        if ($rs_details['material_id'] != '') {
//            echo "<input type='hidden' name='mat_$ctr' value='" . $rs_details['material_id'] . "'>";
//            echo "<input class='input' type='text' name='mat_code_$ctr' value='" . $rs_mat['code'] . "' readonly>";
//        } else {
...
//        }
    ...
    echo "<input type='hidden' name='del_to' value='" . $rs_dt['dt_id'] . "'>";
    echo "<tr>";
    if ($rs_trans['status'] == 'for_evaluation') {
        echo "<td colspan='12' align='center'><input type='checkbox' name='forEval' value='for_evaluation' checked> For Evaluation</td>";
    } else {
        echo "<td colspan='12' align='center'><input type='checkbox' name='forEval' value='for_evaluation'> For Evaluation</td>";
    }
    echo "</tr>";
    echo "<tr>";
    ...
    if ($rs_trans['status'] == 'for_evaluation') {
        echo "<font color='red'>This transaction is for evaluation <a href='submit_for_evaluation.php?trans_id=" . $_GET['trans_id'] . "'>click here</a> to encode the result.</font>";
    }

    $sql_check = mysql_query("SELECT * FROM scale_recdet_forevaluation WHERE trans_id='" . $_GET['trans_id'] . "'");
    $rs_count = mysql_num_rows($sql_check);

    if ($rs_count > 0) {
        echo "This transaction already evaluated.";
    }
