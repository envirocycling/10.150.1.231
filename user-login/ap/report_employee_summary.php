...
$sqli = mysqli_connect("$url_sqli", 'branches', 'enviro101', "$db_name");
                                           // echo "SELECT * from employee_advances WHERE (status = 'issued' or status = 'liquidated') and date_received <= '$to' and branch_id = '".$row_branches['branch_id']."' order by date_received Asc<br>";
                                            $sql_suppAdv = mysqli_query($sqli,"SELECT * from employee_advances WHERE (status = 'issued' or status = 'liquidated') and (date_received >= '$from' AND date_received <= '$to') and branch_id = '".$row_branches['branch_id']."' order by date_received Asc") or die(mysqli_error());
                                        }else{
$sqli = mysqli_connect("$url_sqli", 'efi', 'enviro101', "$db_name");
                                            $sql_suppAdv = mysqli_query($sqli,"SELECT * from employee_advances WHERE (status = 'issued' or status = 'liquidated') and (date_received >= '$from' AND date_received <= '$to') order by date_received Asc") or die(mysqli_error());
                                        }
                       ...
                       <?php
                            ...
                            foreach($branch_arr as $slctd_branch){
                                ...
                                if($row_branches['branch_id'] == 7){
                                    $sql_suppAdv2 = mysqli_query($sqli2,"SELECT * from employee_advances WHERE (status = 'issued' or status = 'liquidated') and (date_received >= '$from' AND date_received <= '$to') and branch_id = '".$row_branches['branch_id']."' order by date_received Asc") or die(mysqli_error());

                                }else{
                                    $sql_suppAdv2 = mysqli_query($sqli2,"SELECT * from employee_advances WHERE (status = 'issued' or status = 'liquidated') and (date_received >= '$from' AND date_received <= '$to') order by date_received Asc") or die(mysqli_error());
                                }
                                ...
                                $row_supp = mysqli_fetch_array($sql_supp);
                                   ...
                                   if($chk == 1){
                                    echo '<tr>';
                                         ...
                                         echo '<td>'. utf8_encode($row_supp['name']) .'</td>';
...
//                                     echo $row_suppAdv2['ref_no'].'='.$date_issuedData.' to '.$current_date.'<br>';
                                   }
                                }
                            }
