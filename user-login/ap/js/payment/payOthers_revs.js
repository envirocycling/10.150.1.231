...
function save() {
    ...
    if (cnfrm === true) {
        ...
        var payee = ($("#payee_new").val()).replace("&","[ampersand]");
        var payee2 = ($("#payee_new2").val()).replace("&","[ampersand]");
        if (payee === '') {
            var cheque_name = ($("#payee").val()).replace("&","[ampersand]");
        } else {
            var cheque_name = ($("#payee_new").val()).replace("&","[ampersand]");
        }
        //non trade new payee cheque start
        var chk_nontrade = Number($("#chk_nontrade").val());
        if(chk_nontrade === 1){
            if (payee2 === '') {
                var cheque_name2 = ($("#payee2").val()).replace("&","[ampersand]");
            } else {
                var cheque_name2 = ($("#payee_new2").val()).replace("&","[ampersand]");
            }
        }else{
            var cheque_name2 = '';
        }
