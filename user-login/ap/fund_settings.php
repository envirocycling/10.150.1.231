...
if (isset($_POST['s_time'])) {
    ...
    $holyDate = $_POST['date'];
    if (mysql_query("UPDATE system_settings SET holy_date = '$holyDate'") or die(mysql_error())) {
        echo '<script>
				alert("Successful.");
			</script>';
    }
                ...
                <div style="margin-left: 30px;">
                    ...
                    <form action="" method="POST" onSubmit="return confirm('Do you want to proceed?');">
                        <table>
                            <tr>
                                <td>Time: </td>
                                <td><input class="input" type="time" name="time" value="<?php if (isset($_POST['submit'])) {
                    echo $_POST['time'];
                } else {
                    echo date('H:i', strtotime($row_cutoff['fund_cutofftime']));
                } ?>" required></td>
...
?>
                    ...
                    <form action="" method="POST" onSubmit="return confirm('Do you want to proceed?');">
                        <table>
                            <tr>
                                <td>WorkingDay Holiday: </td>
                                <td><input class="input" type="date" name="date" value="<?php if (isset($_POST['d_time'])) {
    echo $_POST['date'];
} else {
    echo date('Y-m-d');
} ?>" required style="width:200px;"></td>
                                <td colspan="2" align="center"><input class="submit" type="submit" name="d_time" value="Submit"></td>
                            </tr>
                        </table>
                    </form>
                    <br>
