...
<tr class="data">
            ...
            <th class="data">Payment Number</th>
...
$sql_paid = mysql_query("SELECT * FROM payment WHERE date>='".$_GET['from']."' and date<='".$_GET['to']."' and status!='deleted' and status!='cancelled'");
while ($rs_paid = mysql_fetch_array($sql_paid)) {
    $sql_receiving = mysql_query("SELECT * from scale_receiving WHERE payment_id='".$rs_paid['payment_id']."'") or die (mysql_error());
    while($row_receiving = mysql_fetch_array($sql_receiving)){
        $sql_details = mysql_query("SELECT * FROM scale_receiving_details WHERE trans_id='".$row_receiving['trans_id']."'");
        while ($rs_details = mysql_fetch_array($sql_details)) {
            if($rs_paid['bank_code'] == 'SBC'){
                $cheq = $rs_paid['cheque_no'].'_'.$rs_paid['voucher_no'];
            }else{
                $cheq = $rs_paid['cheque_no'];
            }
            echo "<tr>";
            echo "<td>" . $row_receiving['date'] . "</td>";
            echo "<td>" .$cheq . "</td>";
            echo "<td>".$row_receiving['str_no']."</td>";
            $sql_sup = mysql_query("SELECT * FROM supplier WHERE id='".$row_receiving['supplier_id']."'");
            $rs_sup = mysql_fetch_array($sql_sup);
            echo "<td>".$rs_sup['supplier_id']."_".$rs_sup['supplier_name']."</td>";
            echo "<td>".$row_receiving['plate_number']."</td>";
