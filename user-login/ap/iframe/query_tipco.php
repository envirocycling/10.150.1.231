...
<tr class="data">
 ...
 //        $testrec = mysql_query("SELECT scale.status as status1,supplier.status as status2, scale_details.net_weight, commodity.name FROM scale 
	// INNER JOIN scale_details ON scale.scale_id = scale_details.scale_id
	// INNER JOIN commodity ON scale_details.com_id = commodity.com_id
	// INNER JOIN supplier ON scale.supplier_id=supplier.supplier_id
	//  WHERE (scale.company_id='1' or scale.company_id='2' or scale.company_id='5') and scale.status!='DELETED' and scale.date>='" . $_GET['from'] . "' and scale.date<='" . $_GET['to'] . "' and supplier.owner='EFI'");

 //                $total = 0;

 //        while ($r = mysql_fetch_array($testrec)) {
	// $w = (int) $r['net_weight'];
 //            $total += round($w);

 //        }

        //die(var_dump(round($total/1000)));
...
//die(var_dump(mysql_fetch_array($sql_rec)));
