...
while ($rs_bank2 = mysql_fetch_array($sql_bank2)) {
    ...
    if ($_GET['type'] != 'cancelled') {
        $sql_paid2 = mysql_query("SELECT * FROM payment WHERE bank_code like '%" . $rs_bank2['bank_code'] . "%' and bank_code!='SBC' and type like '%" . $_GET['type'] . "%' and (cheque_date>='" . $_GET['from'] . "' and cheque_date<='" . $_GET['to'] . "') and  status!='deleted'  order by bank_code,date asc");
    } else {
        $sql_paid2 = mysql_query("SELECT * FROM payment WHERE bank_code like '%" . $rs_bank2['bank_code'] . "%' and bank_code!='SBC' and status like '%" . $_GET['type'] . "%' and (cheque_date>='" . $_GET['from'] . "' and cheque_date<='" . $_GET['to'] . "') and  status!='deleted' order by bank_code,date asc");
    }
    while ($rs_paid2 = mysql_fetch_array($sql_paid2)) {
		
		 $myDate = strtotime($rs_paid2['cheque_date']);
		 $myDate2 =strtotime($rs_paid2['date']);
		
		 $myNum = $myDate - $myDate2;
		
		if($myNum != 0){
			$array_chkstatus[$rs_paid2['payment_id']]='post dated';
		}
		
        array_push($array_pay_id, $rs_paid2['payment_id']);
		
        $array_description[$rs_paid2['payment_id']] = $rs_paid2['description'];
		$array_payee[$rs_paid2['payment_id']] = $rs_paid2['cheque_name'];
        $array_bank[$rs_paid2['payment_id']] = $rs_paid2['bank_code'];
        $array_date[$rs_paid2['payment_id']] = $rs_paid2['cheque_date'];
        $array_time[$rs_paid2['payment_id']] = $rs_paid2['time'];
        $array_status[$rs_paid2['payment_id']] = $rs_paid2['status'];
        $array_cheque[$rs_paid2['payment_id']] = $rs_paid2['cheque_no'];

        if ($rs_paid2['status'] != 'cancelled' || $_GET['type'] = 'cancelled') {
...
foreach ($array_pay_id as $id) {
    ...
    echo '<td>' . @$array_time[$id] . '</td>';
    echo '<td>' . @$array_cheque[$id] . '</td>';
    echo '<td>' . @$array_payee[$id] . '</td>';
	echo '<td>' . @$array_description[$id] . '</td>';
    echo '<td>'; if(!empty($array_chkstatus[$id])){echo $array_chkstatus[$id];}else{ echo @$array_status[$id];} echo '</td>';
