...
function save($cheque_date, $trans_data) {
    ...
    while ($rs_save_adj = mysqli_fetch_array($sql_save_adj)) {
        if ($rs_save_adj['adj_type'] != '') {
            ...
            truckPayStatus($rs_save_adj['tp_id'], maxId(), 'paid');
...
function truckPayStatus($tp_id, $payment_id, $status) {
    include '../config.php';
    if ($status == 'paid') {
        mysqli_query($conn, "UPDATE truck_payment SET payment_id='$payment_id',status='paid',date_paid='" . date("Y-m-d H:i:s") . "' WHERE tp_id='$tp_id'");
    } else {
        mysqli_query($conn, "UPDATE truck_payment SET payment_id='',status='',date_paid='0000-00-00 00:00:00' WHERE tp_id='$tp_id'");
    }
}
