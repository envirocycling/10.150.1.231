...
if ($_GET['payment'] == 'canSup') {
    ...
    while ($rs_check = mysql_fetch_array($sql_check)) {
        if ($rs_check['ac_id'] != '' && $rs_check['adj_type'] == 'add') {
    ...
    if ($err == 0) {
        ...
        mysql_query("UPDATE payment SET status='deleted' WHERE payment_id='" . $_POST['payment_id'] . "'");
...
if ($_GET['payment'] == 'canAdv') {
    ...
    if ($rs_check['branch_id'] == '7') {
        $sql_adv_less = mysql_query("SELECT * FROM payment_adjustment INNER JOIN payment ON payment_adjustment.payment_id=payment.payment_id WHERE payment_adjustment.ac_id='" . $_POST['payment_id'] . "' and payment_adjustment.adj_type='deduct' and payment.status!='cancelled' and payment.status!='deleted'");
        $count_adv_less = mysql_num_rows($sql_adv_less);

        $sql_adv_pay = mysql_query("SELECT * FROM adv_payment WHERE ac_id='" . $_POST['payment_id'] . "' and status!='cancelled'");
        $count_adv_pay = mysql_num_rows($sql_adv_pay);
