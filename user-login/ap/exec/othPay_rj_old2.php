...
function save($user_id) {
    ...
    $stmt = mysqli_prepare($conn, "INSERT INTO payment (`bank_code`, `cheque_no`, `voucher_no`, `cheque_name`, `supplier_id`, `description`, `sub_total`, `ts_fee`, `adjustments`, 
            `grand_total`, `account_name`, `account_number`, `type`, `pay_type`, `cancelled_cheque`, `ap`, `verifier`, `signatory`, `remarks`, `charge_to`, `date`, `time`, 
            `cheque_date`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
    $err+= checkStmt($stmt);

    $bind = mysqli_stmt_bind_param($stmt, "ssssisddddsssssssssssss", $rs_save['bank_code'], $rs_save['cheque_no'], $rs_save['voucher_no'], $rs_save['cheque_name'], $rs_save['supplier_id'], $rs_save['description'], $rs_save['sub_total'], $rs_save['ts_fee'], $rs_save['adjustments'], $rs_save['grand_total'], $rs_save['account_name'], $rs_save['account_number'], $rs_save['type'], $rs_save['pay_type'], $rs_save['old_cheque_no'], $rs_save['ap'], $rs_save['verifier'], $rs_save['signatory'], $rs_save['remarks'], $rs_save['charge_to'], date("Y/m/d"), date("H:i"), $rs_save['cheque_date']);
...
function saveTempPay($temp_data, $temp_data_details) {
    ...
    $stmt = mysqli_prepare($conn, "UPDATE `temp_payment` SET `bank_code`=?,`cheque_no`=?,`old_cheque_no`=?,`voucher_no`=?,`cheque_name`=?,`supplier_id`=?,`description`=?,
            `sub_total`=?,`ts_fee`=?,`adjustments`=?,`grand_total`=?,`account_name`=?,`account_number`=?,`type`=?,`pay_type`=?,`ap`=?,`verifier`=?,`signatory`=?,`remarks`=?,
            `charge_to`=?, `cheque_date`=? WHERE user_id=?");
    $bind = mysqli_stmt_bind_param($stmt, "sssssisddddssssssssssi", $temp_data['bank_code'], $temp_data['cheque_no'], $temp_data['old_cheque_no'], $temp_data['voucher_no'], $temp_data['cheque_name'], $temp_data['supplier_id'], $temp_data['description'], $temp_data['sub_total'], $temp_data['ts_fee'], $temp_data['adjustments'], $temp_data['grand_total'], $temp_data['account_name'], $temp_data['account_number'], $temp_data['type'], $temp_data['pay_type'], $temp_data['ap'], $temp_data['verifier'], $temp_data['signatory'], $temp_data['remarks'], $temp_data['charge_to'], $temp_data['cheque_date'], $_SESSION['user_id']);
    $exec = mysqli_stmt_execute($stmt);
...
function update($user_id, $payment_id) {
    ...
    $stmt = mysqli_prepare($conn, "UPDATE payment SET `bank_code`=?, `cheque_no`=?, `voucher_no`=?, `cheque_name`=?, `supplier_id`=?, `description`=?, `sub_total`=?, `ts_fee`=?, `adjustments`=?, `grand_total`=?, `account_name`=?, `account_number`=?, `type`=?, `pay_type`=?, `cancelled_cheque`=?, `ap`=?, `verifier`=?, `signatory`=?, `remarks`=?, `charge_to`=?, `cheque_date`=? WHERE payment_id=?");
    $err+= checkStmt($stmt);

    $bind = mysqli_stmt_bind_param($stmt, "ssssisddddsssssssssssi", $rs_save['bank_code'], $rs_save['cheque_no'], $rs_save['voucher_no'], $rs_save['cheque_name'], $rs_save['supplier_id'], $rs_save['description'], $rs_save['sub_total'], $rs_save['ts_fee'], $rs_save['adjustments'], $rs_save['grand_total'], $rs_save['account_name'], $rs_save['account_number'], $rs_save['type'], $rs_save['pay_type'], $rs_save['old_cheque_no'], $rs_save['ap'], $rs_save['verifier'], $rs_save['signatory'], $rs_save['remarks'], $rs_save['charge_to'], $rs_save['cheque_date'], $payment_id);
    $exec = mysqli_execute($stmt);
