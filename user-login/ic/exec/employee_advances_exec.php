...
if($up == '1'){
	...
	mysql_query("UPDATE employee_advances SET date='$date', branch_id='".$row_branch['branch_id']."', emp_id='$emp_id', amount='$amount', purpose='$purpose', approver='$approver' WHERE ea_id='$ea_id'") or die(mysql_error());
}else{
        if(mysql_query("INSERT INTO employee_advances (emp_id, ref_no, amount, purpose, date, approver, status,prepared_by, branch_id) VALUES ('$emp_id', '$ref_no', '$amount', '$purpose', '$date', '$approver', 'pending','".$_SESSION['ic_id']."','".$row_branch['branch_id']."')") or die(mysql_error())){
