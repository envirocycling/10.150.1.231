...
while ($c < 1) {
    if ($files[$c] != '..' && $files[$c] != '.') {
        ...
        VALUES ('$str_no_new','$date','" . $rs_sup['id'] . "','$delivered_to_id','$plate_number')")or die(mysql_error());
                ...
                $scale_out = mysql_query("INSERT INTO `scale_outgoing`(`str_no`, `date`, `supplier_id`, `branch_id`, `dt_id`, `plate_number`)
        ...
        VALUES ('$str_no_new','$date','" . $rs_sup['id'] . "','" . $rs_branch['branch_id'] . "','$delivered_to_id','$plate_number')")or die(mysql_error());
            ...
            $rs_str_exist = mysql_fetch_array($str_exist);

            if (mysql_num_rows($str_exist) > 0) {

                if ($delivered_to_id == $rs_str_exist['dt_id']) {
                    $str_no_new = $str_no . 'B';
