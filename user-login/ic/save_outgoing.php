...
if (isset($_POST['submit'])) {
    mysql_query("UPDATE scale_outgoing SET str_no='" . $_POST['str_no'] . "',tr_no='" . $_POST['tr_no'] . "',series_no='" . $_POST['series_no'] . "',supplier_id='" . $_POST['supplier_id'] . "',upload='0',up_out='0',checked='1' WHERE trans_id='" . $_POST['trans_id'] . "'");

    $c = $_POST['ctr'];
    $ctr = 0;

    while ($ctr < $c) {
        mysql_query("UPDATE scale_outgoing_details SET material_id='" . $_POST['mat_' . $ctr] . "',mc='" . $_POST['mc' . $ctr] . "',dirt='" . $_POST['dirt' . $ctr] . "',corrected_weight='" . $_POST['corrected_wt' . $ctr] . "',remarks='" . $_POST['remarks' . $ctr] . "', up_sales=0 WHERE detail_id='" . $_POST['detail_id' . $ctr] . "'");
        $ctr++;
    }

    echo "<script>
        alert('Transaction successfully Save.');
        window.close();
        </script>";
} else {

    $sql_trans = mysql_query("SELECT * FROM scale_outgoing WHERE trans_id='" . $_GET['trans_id'] . "'");
    ...
    echo "<center>";
    echo "<br>";
    echo "<form action='save_outgoing.php' method='POST'>";
    ...
    echo "<form action='save_outgoing.php?rec_trans_id=" . $rs_trans['rec_trans_id'] . "' method='POST'>";
    ...
    $ctr = 0;
    $sql_details = mysql_query("SELECT * FROM scale_outgoing_details WHERE trans_id='" . $_GET['trans_id'] . "'");
