    ...
    function save() {
        ...
        if (remarks != '') {
            if (password == passcode) {
                var dataString = 'ctr=' + ctr + '&trans_id=' + trans_id + '&' + mat + '&' + gross + '&' + tare + '&' + net_weight + '&remarks=' + remarks;
                $.ajax({
                    type: "POST",
                ...
                alert('Supervisor Code is Incorrect.');
...
if ($rs_rec['status'] == 'paid') {
    ...
    $sql_pass = mysql_query("SELECT password FROM password WHERE id='1'");
    $rs_pass = mysql_fetch_array($sql_pass);
