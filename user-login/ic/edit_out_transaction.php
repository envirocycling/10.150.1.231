...
if (isset($_POST['submit'])) {
    ...
    if ($count == $ctr) {
        ...
        $sql_trans = mysql_query("SELECT * FROM scale_outgoing WHERE trans_id='$trans_id'");
        $rs_trans = mysql_fetch_array($sql_trans);

        $scale_rec = mysql_query("INSERT INTO `scale_outgoing`(`str_no`, `date`, `supplier_id`, `dt_id`, `plate_number`, `branch_id`)
        VALUES ('" . $rs_trans['str_no'] . "B','" . $rs_trans['date'] . "','" . $rs_trans['supplier_id'] . "','" . $rs_trans['dt_id'] . "','" . $rs_trans['plate_number'] . "', '".$rs_trans['branch_id']."')")or die(mysql_error());
        ...
        while ($c < $ctr) {
            if (isset($_POST['detail_id_' . $c])) {
                mysql_query("UPDATE scale_outgoing_details SET trans_id='" . $rs_out_max['max(trans_id)'] . "', up_sales=0 WHERE detail_id='" . $_POST['detail_id_' . $c] . "'");
            }
            $c++;
        }
        echo "<script>";
    ...
    $date = $rs_rec['date'];
    echo "<center>";
    echo "<h2>Edit Outgoing Transaction</h2>";
    echo "<table>";
    echo "<tr>";
...
//    echo "<td><b>Supplier Name:</b> <u><a rel='facebox' href='edit_sup.php?trans_id=" . $_GET['trans_id'] . "'>" . $rs_sup['supplier_id'] . "_" . $supplier_name . "</a></u>&nbsp;&nbsp;</td>";
    ...
    echo "<form action='edit_out_transaction.php' method='POST'>";
