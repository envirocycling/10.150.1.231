            ...
            <div class="middle">
                ...
                include 'template/menu.php';include 'config.php';
					...
					$row_emp = mysql_fetch_array($sql_emp);
                                        ...
                                        $sql_branch = mysql_query("SELECT * from branches WHERE branch_id='".$row_eadv['branch_id']."'") or die(mysql_error());
                ...
                <div align="center">
                    ...
                    <br>
							...
							<div class="table">
								...
								<div class="row">
									<div class="column">Branch: <span class="text"><?php echo strtoupper($row_branch['branch_name']);?></span></div>
						...
						<tr class="input">
							...
							<?php
								while($row_liquidate = mysql_fetch_array($sql_liquidate)){
									echo '<tr>
											<td  style="width:400px;">'.strtoupper($row_liquidate['details']).'</td>
											<td><span class="num">'.number_format($row_liquidate['amount'],2).'</span></td>
										</tr>';
								}
							?>
						<tr>
							...
							<td><span class="num" id="excess_cash"><?php echo number_format($row_eadv['excess_cash'],2);?></span></td>
                                                ...
                                                <tr>
							<td>RETURNED CASH</td>
							<td><span class="num" id="total_expense"><?php echo number_format($row_eadv['returned_excess_cash'],2);?></span></td>
						</tr>
                                                <tr>
							<td>BALANCE</td>
							<td><span class="num" id="total_expense"><?php $variance= $row_eadv['excess_cash'] - $row_eadv['returned_excess_cash'];echo number_format($variance,2);?></span></td>
						</tr>
                    </table>
                </div>
                <br>
				<div align="right" style="width:70%;"><a href="employee_advances_list.php"><input type="button" value="Back" class="large-submit"></a></div><br>
            </div><!-- .middle-->
