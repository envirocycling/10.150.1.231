...
while ($rs_rec = mysql_fetch_array($sql_rec)) {
    ...
    echo "<td class='data'>";
    if (($rs_rec['status'] == '') && ($_SESSION['user_id'] == 55 || $_SESSION['user_id'] == 40)) {
        echo "<input type='submit' id='b_" . $rs_rec['trans_id'] . "' onclick='openWindow(this.id);' class='button' name='save' value='Save'> ";
    } else if (($rs_rec['status'] == 'generated' || $rs_rec['status'] == 'for_evaluation') &&($_SESSION['user_id'] == 55 || $_SESSION['user_id'] == 40)) {
    ...
    if ($rs_count > 1 && $rs_rec['status'] == '' && ($_SESSION['user_id'] == 55 || $_SESSION['user_id'] == 40)) {
        echo "<button id='" . $rs_rec['trans_id'] . "' onclick='openWindow2(this.id);' class='button'>New STR</button> ";
    }
    //echo "<button id='" . $rs_rec['trans_id'] . "' onclick='openWindow2(this.id);' class='button'>New STR</button> ";
    if ($rs_rec['status'] == 'generated' && ($_SESSION['user_id'] == 55 || $_SESSION['user_id'] == 40)) {
