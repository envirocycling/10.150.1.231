...
if (@$_GET['update'] == 'c_success'){

    mysql_query("UPDATE client_price SET up='1' WHERE up='2'") or die(mysql_error());

}else if (@$_GET['update'] == 'n_success'){

    mysql_query("UPDATE client SET up='1' WHERE up='2'") or die(mysql_error());

}else if (@$_GET['update'] == 'outgoing') {
...
if ($retcode == 200) {
    ...
    $sql_client = mysql_query("SELECT * from client WHERE ( up='0' or up='2' )") or die(mysql_error());


    if (mysql_num_rows($sql_client) > 0) {
        echo "<form action='http://ims.efi.net.ph/client_update.php?update=name' method='POST' name='myForm'>";
        $ctr = 0;
        while($row_client = mysql_fetch_array($sql_client)){
         echo '<input type="text" name="cid_'.$ctr.'" value="'.$row_client['cid'].'">
                <input type="text" name="desc_'.$ctr.'" value="'.$row_client['description'].'">
                <input type="text" name="name_'.$ctr.'" value="'.$row_client['client_name'].'">';
            $ctr++;
        }

        echo "<input type='text' value='$ctr' name='ctr'>";
        mysql_query("UPDATE client SET up='2' WHERE up='0'") or die(mysql_error());
        echo
        "</form>";
        echo "
<script>
...
";
     ...
     $sql_client_price = mysql_query("SELECT * from client_price WHERE (up='0' or up='2') and status='1'") or die(mysql_error());

    if (mysql_num_rows($sql_client_price) > 0) {

        echo "<form action='http://ims.efi.net.ph/client_update.php?update=price' method='POST' name='myForm'>";
        $ctr = 0;
        while($row_client_price = mysql_fetch_array($sql_client_price)){
         echo '<input type="text" name="cid_'.$ctr.'" value="'.$row_client_price['client_id'].'">
                <input type="text" name="material_'.$ctr.'" value="'.$row_client_price['material_id'].'">
                <input type="text" name="price_'.$ctr.'" value="'.$row_client_price['price'].'">
                <input type="text" name="dateeffective_'.$ctr.'" value="'.$row_client_price['date_effective'].'">';
            $ctr++;
        }
        ...
        echo
        "</form>";
        mysql_query("UPDATE client_price SET up='2' WHERE up='0'") or die(mysql_error());
        echo "
<script>
    document.myForm.submit();
</script>
";
	...
	//die(var_dump($row));
        ...
        $select_supplier = mysql_query("SELECT * from branches WHERE branch_id='" . $row['branch_id'] . "' ") or die(mysql_error());
        $select_supplier_row = mysql_fetch_array($select_supplier);
        $branch_ = $select_supplier_row['branch_name'];
        ...
        //die("SELECT * from scale_outgoing_details WHERE trans_id='" . $row['trans_id'] . "'");
...
";
    ...
    $result = mysql_query("SELECT * from scale_receiving WHERE upload='0' and checked='1' and `status` != 'void' Order by trans_id Asc LIMIT 1") or die(mysql_error());
    ...
    if (mysql_num_rows($result) > 0) {
        ...
        while ($select_row = mysql_fetch_array($select)) {
			...
			if($select_row['fe_detail_id'] != 0){
				$fe_sql = mysql_query("SELECT * from scale_recdet_forevaluation WHERE detail_id='".$select_row['fe_detail_id']."'") or die(mysql_error());
				$row_fe_sql = mysql_fetch_array($fe_sql);
				$prev_detail_id = $row_fe_sql['prev_detail_id'];
			}

            $parameter.=$supplier_id . "+" . $supplier_name . "+" . $wp_grade . "+" . $weight . "+" . $date . "+" . $branch . "+" . $id . "+" . $dtld_id . "+" . $month_delivered . "+" . $year_delivered . "+" . $str . "+" . $plate . "+". $prev_detail_id ."|";
        }
    ...
    $result = mysql_query("SELECT * from scale_receiving WHERE up_paper='0' and checked='1' and `status` != 'void' Order by trans_id Asc LIMIT 1") or die(mysql_error());
...
// 			branch
    ...
    // echo "<script>console.log('$retcode');</script>";
