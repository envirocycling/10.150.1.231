...
$url2 = "http://".$row_branch['ip_address']."/ts/fundaddreq_bgchk.php";
...
$sql_ftransfer2 = mysql_query("SELECT * from fund_transfer WHERE up='1' and branch_id='".$row_branch['branch_id']."'") or die(mysql_error());
$row_ftransfer2  = mysql_fetch_array($sql_ftransfer2);
...
$row_fundreq = mysql_fetch_array($sql_fundreq);
			...
			if(mysql_num_rows($sql_fundreq) == 0 && !empty($afr_id)){
			...
			}else if(mysql_num_rows($sql_ftransfer2) > 0){
				//$url2 = "http://192.168.13.5/ts/fundaddreq_bgchk.php";
							
							echo '<form action="'.$url2.'" method="post" name="myForm">
										<input type="text" name="allocated" value="'.$row_ftransfer2['transferred'].'">
										<input type="text" name="date_allocated" value="'.$row_ftransfer2['date_transfer'].'">
										<input type="text" name="date_up" value="'.$row_ftransfer2['date'].'">
										<input type="text" name="update" value="1">
										<input type="text" name="connected" value="1">
								</form>';
					mysql_query("UPDATE system_settings SET fund_updatecontrol='0'") or die(mysql_error());
					mysql_query("UPDATE fund_transfer SET up='' WHERE ft_id='".$row_ftransfer2['ft_id']."'") or die(mysql_error());
							echo '<script>
									document.myForm.submit();
								</script>';
